...
public static class AudioStorage
{
    ...
    public static async Task RegisterSweepSemitoneAsync(double freqFrom, double freqTo, bool exp, double duration, double amp = 0.5, SweepChanneldOrder channeldOrder = SweepChanneldOrder.Both, int sampleRate = 44100)
    {
        ...
        {
            await RegisterWave(Helper.FreqConverters.NoteNumberToHzEqualTemperament(Math.Floor(start)), duration, amp, amp, WaveKinds.Sine, sampleRate);
            return;
        }
        else if (start > end)
        {
